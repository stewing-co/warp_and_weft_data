...
get_follow_relationships <- function(db_path) {
  ...
  get_all_follows <- function(endpoint_url, user_did) {
    ...
    repeat {
      ...
      if (resp_status(resp) >= 400) {
        message("An error occurred fetching data: ", resp_status_desc(resp))
  ...
  followers_df <- get_all_follows("https://bsky.social/xrpc/app.bsky.graph.getFollowers", self_did)
  
  # Get following
  message("Fetching users you are following...")
  following_df <- get_all_follows("https://bsky.social/xrpc/app.bsky.graph.getFollows", self_did)
...
compute_sentiment_scores_for_new_posts <- function(db_path) {
  ...
  dbWriteTable(con, "post_sentiment_scores", post_sentiment_scores, append = TRUE)
...
unfollow_users_based_on_criteria <- function(db_path) {
  ...
  users_with_zero_posts <- dbGetQuery(con, "
    SELECT handle
  ...
  users_with_low_sentiment <- dbGetQuery(con, "
  ...
  users_to_unfollow <- unique(c(users_with_zero_posts$handle, users_with_low_sentiment$handle))
  ...
  get_all_followers <- function(session, user_did) {
    ...
    repeat {
      ...
      if (resp_status(resp) >= 400) {
        message("An error occurred fetching followers: ", resp_status_desc(resp))
        print(resp_body_string(resp))
        break
      }
  ...
  get_follow_records <- function(session, repo_did) {
    ...
    repeat {
      ...
      if (resp_status(resp) >= 400) {
        message("An error occurred fetching follow records: ", resp_status_desc(resp))
        print(resp_body_string(resp))
        break
      }
  ...
  if (nrow(missing_handles) > 0) {
    ...
    follow_records_df <- follow_records_df %>%
      left_join(resolved_map, by = "subject_did", suffix = c("", ".resolved")) %>%
  ...
  unfollow_user <- function(session, repo_did, rkey) {
    ...
    if (resp_status(resp) >= 400) {
      message("Failed to unfollow rkey ", rkey, ": ", resp_status_desc(resp))
      print(resp_body_string(resp))
    } else {
...
create_histogram_of_avg_sentiment <- function(db_path) {
  ...
  avg_sentiment_df <- dbGetQuery(con, "SELECT author_handle AS handle, average_sentiment FROM wwd_following_average_sentiment")
  ...
  ggplot(merged_df, aes(x = average_sentiment)) +
    ...
    labs(
      title = "Histogram of Average Sentiment Scores of Remaining Follows",
