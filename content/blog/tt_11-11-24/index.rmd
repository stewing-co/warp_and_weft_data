...
ggplot(type_counts, aes(x = type, y = n, fill = type)) +
  ...
  scale_fill_manual(values = c(
    "State" = "#8b4513",         # Example color for "State"
    "Outlying area" = "#D2B48C", # Example color for "Outlying area"
    "District" = "#4B2E2A"       # Example color for "District"
  )) +                                                               # Customize colors based on 'type'
  theme_minimal() +
  theme(
    plot.background = element_rect(fill = "#f5deb3", color = NA),    # Parchment-style background
    panel.grid.major = element_blank(),                              # Remove major grid lines for a clean look
    panel.grid.minor = element_blank(),                              # Remove minor grid lines
    panel.background = element_blank(),
    plot.title = element_text(size = 20, face = "bold", family = "pirata", hjust = 0.5, color = "#4b2e2a"),
    axis.title.x = element_text(size = 14, face = "bold", color = "#4b2e2a", family = "pirata"),
    axis.title.y = element_text(size = 14, face = "bold", color = "#4b2e2a", family = "pirata"),
    axis.text.x = element_text(size = 12, color = "#4b2e2a", family = "pirata"),
    axis.text.y = element_text(size = 12, color = "#4b2e2a", family = "pirata"),
    legend.position = "none"                                         # Hide legend since colors are clear in the bar chart
  ) +
  labs(
